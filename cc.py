...
def token(text):
    if text in keyword:
        ...
        y=text
        a="IDENTIFIER" 
    elif re.match(r"^'[a-zA-Z0-9]'$|^'[!@#$%^&*()-=+{}|;:<>,.?/']'$|^'\\[\'\"\\]'$|^'\\[ntrafb0]'$",text):
        y=text
        a="CHAR CONSTANT"
    elif re.match("^\"([a-zA-Z0-9]|(\\\\\\\\)|[!@#$%^&()-=+{}|;:<>,.?/']|\\[|\\]|_|\\\\[nrtfab0]|\\\\\"|\\\\')\"$",text):
        y=text
        a="STRING CONSTANT"
    else:
        a="Invalid Token"
...
#string="79.88.66.x a.b.c "
# string=
#string="alpha a = b \nfunc void sum(alpha a,alpha b){alpha a=b\n}\nd=2+c\nif(a<b){r=t * 4\n}\nelse{a=2+a\n}\nspan(a==b){s=5 * c\n}\na=2\niterate i in range(a)\na=3\niterate i in range(5){\nia new b()\n}\na++\na+=b\nfunc dec mul(alpha a=b,dec c=2){a.b.c.fn(2)}/aa \n"
#FLoat re change kr dyna
string = 'digit result = a().aa123aaa \ndigit result = 150 \ndigit result = 200 \ndigit result = 300 \ndigit result = 400 \n'
#filing
